...
const DOM = {
    ...
    timeGap: document.getElementById("time-gap")
};
/**
 * Listeners
*/
...
Array.from(DOM.inputContainer).forEach(container => {
    container.addEventListener("change", (e) => {
        ...
        switch (input.id) {
            case "set-to":
                Counter.setNumber(parseInt(input.value));
                break;
            case "step":
                Counter.setStep(parseInt(input.value));
                break;
            case "time-gap":
                Counter.setTimeGap(parseFloat(input.value));
                break;
        }
    });
});
